...
class DiscretizedAcrobot:
  ...
  def __init__(self,
    ...
    self.transform_obs = lambda obs: (
        np.ravel_multi_index((
            np.clip(np.digitize(obs[0], np.linspace(*self.angle_range, self.angle_1_bins)) - 1, 0,
                    self.angle_1_bins - 1),
            np.clip(np.digitize(obs[1], np.linspace(*self.angle_range, self.angle_2_bins)) - 1, 0,
                    self.angle_2_bins - 1),
            np.clip(np.digitize(obs[2], np.linspace(*self.velocity_1_range, self.angular_vel_1_bins)) - 1, 0,
                    self.angular_vel_1_bins - 1),
            np.clip(np.digitize(obs[3], np.linspace(*self.velocity_2_range, self.angular_vel_2_bins)) - 1, 0,
                    self.angular_vel_2_bins - 1),
        ), (self.angle_1_bins, self.angle_2_bins, self.angular_vel_1_bins, self.angular_vel_2_bins))
      )
  ...
  def index_to_state(self, index):
        ...
        totals = [self.angle_1_bins, self.angle_2_bins, self.angular_vel_1_bins, self.angular_vel_2_bins]
  ...
  def compute_next_state(self, angle_1_idx, angle_2_idx, vel_1_idx, vel_2_idx, action):
    ...
    new_angle_1_idx = np.clip(np.digitize(new_state[0], np.linspace(*self.angle_range, self.angle_1_bins)) - 1, 0,
                    self.angle_1_bins - 1)
    new_angle_2_idx = np.clip(np.digitize(new_state[1], np.linspace(*self.angle_range, self.angle_2_bins)) - 1, 0,
                    self.angle_2_bins - 1)
    new_vel_1_idx = np.clip(np.digitize(new_state[2], np.linspace(*self.velocity_1_range, self.angular_vel_1_bins)) - 1, 0,
                    self.angular_vel_1_bins - 1)
    new_vel_2_idx = np.clip(np.digitize(new_state[3], np.linspace(*self.velocity_2_range, self.angular_vel_2_bins)) - 1, 0,
                    self.angular_vel_2_bins - 1)

    new_state_idx = np.ravel_multi_index((new_angle_1_idx, new_angle_2_idx, new_vel_1_idx, new_vel_2_idx),
                                         (self.angle_1_bins, self.angle_2_bins, self.angular_vel_1_bins, self.angular_vel_2_bins))
